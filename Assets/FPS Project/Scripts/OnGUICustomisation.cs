...
public class OnGUICustomisation : MonoBehaviour
{
    ...
    private void Update() { if(Input.GetKeyDown(KeyCode.F2)) Debugging.EnableDebugMode(); }
    ...
    private void OnGUI()
    {
        ...
        for(int i = 0; i < pSC.characterStats.Length; i++)
        {
            ...
            string boxText = $"{_stats.baseStatsName}: {(_stats.baseStats + _stats.tempStats)}";
            Rect boxTexture = new Rect(position3.x, (position3.y * i), scale3.x, scale3.y);
