    ...
    public class Gun : MonoBehaviour
    {
        ...
        protected virtual void Shoot()
        {
            if (Time.time > nextFire)
            {
                ...
                {
                    // sets the target to the camera position plus rayOrigin * weapon range
                    Vector3 target = rayOrigin + (fpsCam.transform.forward * weaponRange);
                    Transform setTarget = null;
                    // if the target is found sets the bullets target to match
